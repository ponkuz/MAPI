...
Direct `run_event_study()` defaults remain permissive for compatibility: confidence and quality minima are zero, and frequency compatibility is not required. `run_configured_event_study()` applies the CLI-equivalent event eligibility and configured score, direction, cost, horizon, and bootstrap policy. It does not create the CLI chronological holdout split, so programmatic callers must pass `evaluation_mask` for holdout-only evaluation. Existing direct callers are not silently migrated.
