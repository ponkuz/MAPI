...
Direct `run_event_study()` defaults remain permissive for compatibility: confidence and quality minima are zero, and frequency compatibility is not required. `run_configured_event_study()` is the protected programmatic entry point that applies the same research eligibility, score, direction, cost, horizon, and bootstrap settings as the CLI configuration. Existing direct callers are not silently migrated.
